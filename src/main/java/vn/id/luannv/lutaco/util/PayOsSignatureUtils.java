...
public class PayOsSignatureUtils {
    public static boolean verify(PayOsWebhookRequest req, String checksumKey) {
        try {
            ...
            log.debug("[system]: Generated signature for webhook: {}", expectedSignature);

            return expectedSignature.equals(req.getSignature());

        } catch (Exception e) {
            log.error("[system]: Failed to verify PayOS webhook signature. Error: {}", e.getMessage(), e);
            return false;
        }
    }
    ...
    public static String generateHmacSha256ForCreatePaymentRequest(PayOSRequest request, String checksumKey) {
        ...
        log.debug("[system]: Raw data for PayOS payment request signature generation: {}", rawData);
    ...
    private static String hmacSha256(String data, String key) {
        try {
            ...
            log.error("[system]: HMAC SHA256 signature generation failed. Error: {}", e.getMessage(), e);
