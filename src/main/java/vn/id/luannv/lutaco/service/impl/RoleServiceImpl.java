...
public class RoleServiceImpl implements RoleService {
    ...
    public Page<Role> literalSearch(String name, Integer page, Integer size) {
        ...
        Page<Role> roles = roleRepository.findByCodeContainingIgnoreCase(userType, pageable);
    ...
    public Role getByRoleCode(UserType userType) {
        return roleRepository.findByCode(userType)
                .orElseThrow(() -> {
                    log.warn("[{}]: Role with code {} not found.", SecurityUtils.getCurrentUsername(), userType);
                    return new BusinessException(
                            ErrorCode.ENTITY_NOT_FOUND,
                            Map.of("code", userType.name())
                    );
                });
    }
}