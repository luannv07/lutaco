...
public interface PayOSRepository extends JpaRepository<PayOS, Long> {
    @Query("SELECT p FROM PayOS p WHERE p.user.id = :userId")
    List<PayOS> findByUserId(@Param("userId") Long userId);
    ...
    Long getUserIdByOrderCode(@Param("orderCode") Integer orderCode);
